// login
export const LOGIN_ERROR = 'LOGIN_ERROR';
export const LOGIN_LOADING = 'LOGIN_LOADING';
export const LOGIN_SUCCESS = 'LOGIN_SUCCESS';
export const LOGIN_RESET = 'LOGIN_RESET';

// register
export const REGISTER_ERROR = 'REGISTER_ERROR';
export const REGISTER_LOADING = 'REGISTER_LOADING';
export const REGISTER_SUCCESS = 'REGISTER_SUCCESS';
export const REGISTER_RESET = 'REGISTER_RESET';

// register
export const GET_CASES_ERROR = 'GET_CASES_ERROR';
export const GET_CASES_LOADING = 'GET_CASES_LOADING';
export const GET_CASES_SUCCESS = 'GET_CASES_SUCCESS';
export const GET_CASES_RESET = 'GET_CASES_RESET';

// add complaint
export const ADD_COMPLAINT_ERROR = 'ADD_COMPLAINT_ERROR';
export const ADD_COMPLAINT_LOADING = 'ADD_COMPLAINT_LOADING';
export const ADD_COMPLAINT_SUCCESS = 'ADD_COMPLAINT_SUCCESS';
export const ADD_COMPLAINT_RESET = 'ADD_COMPLAINT_RESET';

// add FEEDBACK
export const ADD_FEEDBACK_ERROR = 'ADD_FEEDBACK_ERROR';
export const ADD_FEEDBACK_LOADING = 'ADD_FEEDBACK_LOADING';
export const ADD_FEEDBACK_SUCCESS = 'ADD_FEEDBACK_SUCCESS';
export const ADD_FEEDBACK_RESET = 'ADD_FEEDBACK_RESET';

// change pass
export const CHANGE_PASSWORD_ERROR = 'CHANGE_PASSWORD_ERROR';
export const CHANGE_PASSWORD_LOADING = 'CHANGE_PASSWORD_LOADING';
export const CHANGE_PASSWORD_SUCCESS = 'CHANGE_PASSWORD_SUCCESS';
export const CHANGE_PASSWORD_RESET = 'CHANGE_PASSWORD_RESET';

// detail complaint
export const DETAIL_COMPLAINT_ERROR = 'DETAIL_COMPLAINT_ERROR';
export const DETAIL_COMPLAINT_LOADING = 'DETAIL_COMPLAINT_LOADING';
export const DETAIL_COMPLAINT_SUCCESS = 'DETAIL_COMPLAINT_SUCCESS';
export const DETAIL_COMPLAINT_RESET = 'DETAIL_COMPLAINT_RESET';

// list events
export const LIST_EVENT_ERROR = 'LIST_EVENT_ERROR';
export const LIST_EVENT_LOADING = 'LIST_EVENT_LOADING';
export const LIST_EVENT_SUCCESS = 'LIST_EVENT_SUCCESS';
export const LIST_EVENT_RESET = 'LIST_EVENT_RESET';

// forgot pass
export const FORGOT_PASSWORD_ERROR = 'FORGOT_PASSWORD_ERROR';
export const FORGOT_PASSWORD_LOADING = 'FORGOT_PASSWORD_LOADING';
export const FORGOT_PASSWORD_SUCCESS = 'FORGOT_PASSWORD_SUCCESS';
export const FORGOT_PASSWORD_RESET = 'FORGOT_PASSWORD_RESET';